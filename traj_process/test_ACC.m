...
if(exist('netGroup','var'))
    try
        Acc_Check_sl=check_recall('seq_long',location,netGroup,classes,[5 9],'Y');
        fig = Draw_CM(Acc_Check_sl,classes,3);
        if exist('path','var')
            save_adress = [path '\Acc_Check\ACC_Check_long.mat'];
            save(save_adress,'Acc_Check_sl');
            for h=1:(numel(fig)/2)
                saveas(fig{h,1},[path '\Acc_Check\confusionchart_long_' num2str(h) '.png']);
                saveas(fig{h,2},[path '\Acc_Check\roc_long_' num2str(h) '.png']);
            end
        end
    catch
        error = error+1;
